/* Mesh connection
              GND           GND         GND          GND
          ===================================================
    GND  | router_0_3 | router_1_3 | router_2_3 | router_3_3 | GND
          ===================================================
    GND  | router_0_2 | router_1_2 | router_2_2 | router_3_2 | GND
          ===================================================
    GND  | router_0_1 | router_1_1 | router_2_1 | router_3_1 | GND
          ===================================================
    GND  | router_0_0 | router_1_0 | router_2_0 | router_3_0 | GND
          ===================================================
              GND           GND         GND          GND
        (x,y) = (0,0) (x,y) = (1,0) (x,y) = (2,0) (x,y) = (3,0)
*/


module mesh_row_2 #(
    parameter PACKET_WIDTH = 64
) (
    input clk, reset,
    
    // **Top Signals**
    input snro_0_2, nssi_0_2,
    input snro_1_2, nssi_1_2,
    input snro_2_2, nssi_2_2,
    input snro_3_2, nssi_3_2,

    input [63:0] nsdi_0_2,
    input [63:0] nsdi_1_2,
    input [63:0] nsdi_2_2,
    input [63:0] nsdi_3_2,

    output snso_0_2, nsri_0_2,
    output snso_1_2, nsri_1_2,
    output snso_2_2, nsri_2_2,
    output snso_3_2, nsri_3_2,

    output [63:0] sndo_0_2,
    output [63:0] sndo_1_2,
    output [63:0] sndo_2_2,
    output [63:0] sndo_3_2,

    // **Bottom Signals**
    input snsi_0_2, nsro_0_2,
    input snsi_1_2, nsro_1_2,
    input snsi_2_2, nsro_2_2,
    input snsi_3_2, nsro_3_2,

    input [63:0] sndi_0_2,
    input [63:0] sndi_1_2,
    input [63:0] sndi_2_2,
    input [63:0] sndi_3_2,

    output snri_0_2, nsso_0_2,
    output snri_1_2, nsso_1_2,
    output snri_2_2, nsso_2_2,
    output snri_3_2, nsso_3_2,

    output [63:0] nsdo_0_2,
    output [63:0] nsdo_1_2,
    output [63:0] nsdo_2_2,
    output [63:0] nsdo_3_2,
    
// CPU tb signals for x_2

    // For node 0_2
    input  [0:31] node_0_2_inst_in,
    input  [0:63] node_0_2_d_in,
    output [0:31] node_0_2_pc_out,
    output [0:63] node_0_2_d_out,
    output [0:31] node_0_2_addr_out,
    output        node_0_2_memWrEn,
    output        node_0_2_memEn,
    
    // For node 1_2
    input  [0:31] node_1_2_inst_in,
    input  [0:63] node_1_2_d_in,
    output [0:31] node_1_2_pc_out,
    output [0:63] node_1_2_d_out,
    output [0:31] node_1_2_addr_out,
    output        node_1_2_memWrEn,
    output        node_1_2_memEn,
    
    // For node 2_2
    input  [0:31] node_2_2_inst_in,
    input  [0:63] node_2_2_d_in,
    output [0:31] node_2_2_pc_out,
    output [0:63] node_2_2_d_out,
    output [0:31] node_2_2_addr_out,
    output        node_2_2_memWrEn,
    output        node_2_2_memEn,
    
    // For node 3_2
    input  [0:31] node_3_2_inst_in,
    input  [0:63] node_3_2_d_in,
    output [0:31] node_3_2_pc_out,
    output [0:63] node_3_2_d_out,
    output [0:31] node_3_2_addr_out,
    output        node_3_2_memWrEn,
    output        node_3_2_memEn

); 

  // Row 0
  // router 0_2 to 1_2
  wire ccw_si_02_so_12;
  wire ccw_ri_02_ro_12;
  wire [63:0] ccw_di_02_do_12;

  wire cw_so_02_si_12;
  wire cw_ro_02_ri_12;
  wire [63:0] cw_do_02_di_12;

  // router 1_2 to 2_2
  wire ccw_si_12_so_22;
  wire ccw_ri_12_ro_22;
  wire [63:0] ccw_di_12_do_22;

  wire cw_so_12_si_22;
  wire cw_ro_12_ri_22;
  wire [63:0] cw_do_12_di_22;

  // router 2_2 to 3_2
  wire ccw_si_22_so_32;
  wire ccw_ri_22_ro_32;
  wire [63:0] ccw_di_22_do_32;

  wire cw_so_22_si_32;
  wire cw_ro_22_ri_32;
  wire [63:0] cw_do_22_di_32;

  // Router to NIC
  // Router 0_2
  wire net_so_pesi_02;
  wire net_ro_peri_02;
  wire [63:0] net_do_pedi_02;
  wire net_polarity_02;
  wire net_si_peso_02;
  wire net_ri_pero_02;
  wire [63:0] net_di_pedo_02;

  // Router 1_2
  wire net_so_pesi_12;
  wire net_ro_peri_12;
  wire [63:0] net_do_pedi_12;
  wire net_polarity_12;
  wire net_si_peso_12;
  wire net_ri_pero_12;
  wire [63:0] net_di_pedo_12;

  // Router 2_2
  wire net_so_pesi_22;
  wire net_ro_peri_22;
  wire [63:0] net_do_pedi_22;
  wire net_polarity_22;
  wire net_si_peso_22;
  wire net_ri_pero_22;
  wire [63:0] net_di_pedo_22;

  // Router 3_2
  wire net_so_pesi_32;
  wire net_ro_peri_32;
  wire [63:0] net_do_pedi_32;
  wire net_polarity_32;
  wire net_si_peso_32;
  wire net_ri_pero_32;
  wire [63:0] net_di_pedo_32;

  // NIC wires for x_2

  // For NIC of node 0_2
  wire        nicEn_0_2;
  wire        nicEnWR_0_2;
  wire [1:0]  nic_addr_0_2;
  wire [63:0] nic_d_in_0_2;
  wire [63:0] nic_d_out_0_2;

  // For NIC of node 1_2
  wire        nicEn_1_2;
  wire        nicEnWR_1_2;
  wire [1:0]  nic_addr_1_2;
  wire [63:0] nic_d_in_1_2;
  wire [63:0] nic_d_out_1_2;

  // For NIC of node 2_2
  wire        nicEn_2_2;
  wire        nicEnWR_2_2;
  wire [1:0]  nic_addr_2_2;
  wire [63:0] nic_d_in_2_2;
  wire [63:0] nic_d_out_2_2;

  // For NIC of node 3_2
  wire        nicEn_3_2;
  wire        nicEnWR_3_2;
  wire [1:0]  nic_addr_3_2;
  wire [63:0] nic_d_in_3_2;
  wire [63:0] nic_d_out_3_2;


  router router_0_2 (
    .clk (clk),
    .reset(reset),
    .router_position(0), 
    .polarity_out(net_polarity_02),
    // Left side, no connections
    .cwsi(), .cwri(), .cwdi(), .ccwso(), .ccwro(), .ccwdo(),

    // Right side CW out and CCW in
    .ccwsi(ccw_si_02_so_12), .ccwri(ccw_ri_02_ro_12), .ccwdi(ccw_di_02_do_12),
    .cwso(cw_so_02_si_12), .cwro(cw_ro_02_ri_12), .cwdo(cw_do_02_di_12),

    // Bottom, no connections
    .snsi(snsi_0_2), .snri(snri_0_2), .sndi(sndi_0_2), .nsso(nsso_0_2), .nsro(nsro_0_2), .nsdo(nsdo_0_2),

    // Top side
    .snso(snso_0_2), .snro(snro_0_2), .sndo(sndo_0_2), .nssi(nssi_0_2), .nsri(nsri_0_2), .nsdi(nsdi_0_2),

    // PE input/output to NIC
    .pesi(net_so_pesi_02), .pedi(net_do_pedi_02), .peri(net_ro_peri_02),
    .peso(net_si_peso_02), .pedo(net_di_pedo_02), .pero(net_ri_pero_02)
  );

  // NIC module instantiation for 0_2
  nic #(
      .PACKET_WIDTH(PACKET_WIDTH)
  ) nic_0_2 (
      .clk(clk),
      .reset(reset),
  
      // CPU-NIC Interface
      .addr(nic_addr_0_2),
      .d_in(nic_d_in_0_2),
      .d_out(nic_d_out_0_2),
      .nicEn(nicEn_0_2),
      .nicEnWR(nicEnWR_0_2),

  
      // Router-NIC Interface
      .net_si(net_si_peso_02),
      .net_so(net_so_pesi_02),
      .net_ri(net_ri_pero_02),
      .net_ro(net_ro_peri_02),
  
      .net_di(net_di_pedo_02),
      .net_do(net_do_pedi_02),
      .net_polarity(net_polarity_02)
  );
      
    reg [3:0] cpu_0_2_gclk_count;
    wire cpu_0_2_gclk_start;
    reg cpu_0_2_gclk_count_done;
    
    always @(posedge clk or posedge reset) begin
        if (reset) begin
            cpu_0_2_gclk_count <= 3'd0;
            cpu_0_2_gclk_count_done <= 1'b0;
        end else if (cpu_0_2_gclk_start) begin
            if (cpu_0_2_gclk_count < 3'd5) begin
                cpu_0_2_gclk_count <= cpu_0_2_gclk_count + 1;
                cpu_0_2_gclk_count_done <= 1'b0;
            end else begin
                cpu_0_2_gclk_count_done <= 1'b1;
            end
        end else begin
            cpu_0_2_gclk_count <= 3'd0;
            cpu_0_2_gclk_count_done <= 1'b0;
        end
    end

    wire cpu_0_2_clk_gate_en;
    wire cpu_0_2_gclk;
    assign cpu_0_2_clk_gate_en = !(node_0_2_inst_in == 64'b0);

    clk_gate_latch cpu_0_2_clkgate (
        .CLK(clk), 
        .EN(cpu_0_2_clk_gate_en), 
        .GCLK(cpu_0_2_gclk)
    );

    assign cpu_0_2_gclk_start = ~cpu_0_2_clk_gate_en;

four_stage_processor cpu_0_2 (
    //.clk(clk),
    .clk(cpu_0_2_gclk),

    .reset(reset),
    .inst_in(node_0_2_inst_in),
    .d_in(node_0_2_d_in),
    .pc_out(node_0_2_pc_out),
    .d_out(node_0_2_d_out),
    .addr_out(node_0_2_addr_out),
    .memWrEn(node_0_2_memWrEn),
    .memEn(node_0_2_memEn),
    .nicEn(nicEn_0_2),
    .nicWrEn(nicEnWR_0_2),
    .addr_nic(nic_addr_0_2),
    .d_in_nic(nic_d_in_0_2),
    .d_out_nic(nic_d_out_0_2)
);
  router router_1_2 (
    .clk (clk),
    .reset(reset),
    .router_position(0), 
    .polarity_out(net_polarity_12),
    // Left side
    .cwsi(cw_so_02_si_12), .cwri(cw_ro_02_ri_12), .cwdi(cw_do_02_di_12),
    .ccwso(ccw_si_02_so_12), .ccwro(ccw_ri_02_ro_12), .ccwdo(ccw_di_02_do_12),

    // Right side CW out and CCW in
    .ccwsi(ccw_si_12_so_22), .ccwri(ccw_ri_12_ro_22), .ccwdi(ccw_di_12_do_22),
    .cwso(cw_so_12_si_22), .cwro(cw_ro_12_ri_22), .cwdo(cw_do_12_di_22),

    // Bottom, no connections
    .snsi(snsi_1_2), .snri(snri_1_2), .sndi(sndi_1_2), .nsso(nsso_1_2), .nsro(nsro_1_2), .nsdo(nsdo_1_2),

    // Top side
    .snso(snso_1_2), .snro(snro_1_2), .sndo(sndo_1_2), .nssi(nssi_1_2), .nsri(nsri_1_2), .nsdi(nsdi_1_2),

    // PE input/output to NIC
    .pesi(net_so_pesi_12), .pedi(net_do_pedi_12), .peri(net_ro_peri_12),
    .peso(net_si_peso_12), .pedo(net_di_pedo_12), .pero(net_ri_pero_12)
  );

  // NIC module instantiation for 0_2
  nic #(
      .PACKET_WIDTH(PACKET_WIDTH)
  ) nic_1_2 (
      .clk(clk),
      .reset(reset),
  
      // CPU-NIC Interface
      .addr(nic_addr_1_2),
      .d_in(nic_d_in_1_2),
      .d_out(nic_d_out_1_2),
      .nicEn(nicEn_1_2),
      .nicEnWR(nicEnWR_1_2),

  
      // Router-NIC Interface
      .net_si(net_si_peso_12),
      .net_so(net_so_pesi_12),
      .net_ri(net_ri_pero_12),
      .net_ro(net_ro_peri_12),
  
      .net_di(net_di_pedo_12),
      .net_do(net_do_pedi_12),
      .net_polarity(net_polarity_12)
  );
  
      reg [3:0] cpu_1_2_gclk_count;
    wire cpu_1_2_gclk_start;
    reg cpu_1_2_gclk_count_done;
    
    always @(posedge clk or posedge reset) begin
        if (reset) begin
            cpu_1_2_gclk_count <= 3'd0;
            cpu_1_2_gclk_count_done <= 1'b0;
        end else if (cpu_1_2_gclk_start) begin
            if (cpu_1_2_gclk_count < 3'd5) begin
                cpu_1_2_gclk_count <= cpu_1_2_gclk_count + 1;
                cpu_1_2_gclk_count_done <= 1'b0;
            end else begin
                cpu_1_2_gclk_count_done <= 1'b1;
            end
        end else begin
            cpu_1_2_gclk_count <= 3'd0;
            cpu_1_2_gclk_count_done <= 1'b0;
        end
    end

    wire cpu_1_2_clk_gate_en;
    wire cpu_1_2_gclk;
    assign cpu_1_2_clk_gate_en = !(node_1_2_inst_in == 64'b0);

    clk_gate_latch cpu_1_2_clkgate (
        .CLK(clk), 
        .EN(cpu_1_2_clk_gate_en), 
        .GCLK(cpu_1_2_gclk)
    );

    assign cpu_1_2_gclk_start = ~cpu_1_2_clk_gate_en;

  
    four_stage_processor cpu_1_2 (
        //.clk(clk),
        .clk(cpu_1_2_gclk),

        .reset(reset),
        .inst_in(node_1_2_inst_in),
        .d_in(node_1_2_d_in),
        .pc_out(node_1_2_pc_out),
        .d_out(node_1_2_d_out),
        .addr_out(node_1_2_addr_out),
        .memWrEn(node_1_2_memWrEn),
        .memEn(node_1_2_memEn),
        .nicEn(nicEn_1_2),
        .nicWrEn(nicEnWR_1_2),
        .addr_nic(nic_addr_1_2),
        .d_in_nic(nic_d_in_1_2),
        .d_out_nic(nic_d_out_1_2)
    );

    router router_2_2 (
    .clk (clk),
    .reset(reset),
    .router_position(0), 
    .polarity_out(net_polarity_22),
    // Left side
    .cwsi(cw_so_12_si_22), .cwri(cw_ro_12_ri_22), .cwdi(cw_do_12_di_22),
    .ccwso(ccw_si_12_so_22), .ccwro(ccw_ri_12_ro_22), .ccwdo(ccw_di_12_do_22),

    // Right side CW out and CCW in
    .ccwsi(ccw_si_22_so_32), .ccwri(ccw_ri_22_ro_32), .ccwdi(ccw_di_22_do_32),
    .cwso(cw_so_22_si_32), .cwro(cw_ro_22_ri_32), .cwdo(cw_do_22_di_32),

    // Bottom, no connections
    .snsi(snsi_2_2), .snri(snri_2_2), .sndi(sndi_2_2), .nsso(nsso_2_2), .nsro(nsro_2_2), .nsdo(nsdo_2_2),

    // Top side
    .snso(snso_2_2), .snro(snro_2_2), .sndo(sndo_2_2), .nssi(nssi_2_2), .nsri(nsri_2_2), .nsdi(nsdi_2_2),

    // PE input/output to NIC
    .pesi(net_so_pesi_22), .pedi(net_do_pedi_22), .peri(net_ro_peri_22),
    .peso(net_si_peso_22), .pedo(net_di_pedo_22), .pero(net_ri_pero_22)
  );

  // NIC module instantiation for 0_2
  nic #(
      .PACKET_WIDTH(PACKET_WIDTH)
  ) nic_2_2 (
      .clk(clk),
      .reset(reset),
  
      // CPU-NIC Interface
      .addr(nic_addr_2_2),
      .d_in(nic_d_in_2_2),
      .d_out(nic_d_out_2_2),
      .nicEn(nicEn_2_2),
      .nicEnWR(nicEnWR_2_2),

  
      // Router-NIC Interface
      .net_si(net_si_peso_22),
      .net_so(net_so_pesi_22),
      .net_ri(net_ri_pero_22),
      .net_ro(net_ro_peri_22),
  
      .net_di(net_di_pedo_22),
      .net_do(net_do_pedi_22),
      .net_polarity(net_polarity_22)
  );
  
      reg [3:0] cpu_2_2_gclk_count;
    wire cpu_2_2_gclk_start;
    reg cpu_2_2_gclk_count_done;
    
    always @(posedge clk or posedge reset) begin
        if (reset) begin
            cpu_2_2_gclk_count <= 3'd0;
            cpu_2_2_gclk_count_done <= 1'b0;
        end else if (cpu_2_2_gclk_start) begin
            if (cpu_2_2_gclk_count < 3'd5) begin
                cpu_2_2_gclk_count <= cpu_2_2_gclk_count + 1;
                cpu_2_2_gclk_count_done <= 1'b0;
            end else begin
                cpu_2_2_gclk_count_done <= 1'b1;
            end
        end else begin
            cpu_2_2_gclk_count <= 3'd0;
            cpu_2_2_gclk_count_done <= 1'b0;
        end
    end

    wire cpu_2_2_clk_gate_en;
    wire cpu_2_2_gclk;
    assign cpu_2_2_clk_gate_en = !(node_2_2_inst_in == 64'b0);

    clk_gate_latch cpu_2_2_clkgate (
        .CLK(clk), 
        .EN(cpu_2_2_clk_gate_en), 
        .GCLK(cpu_2_2_gclk)
    );

    assign cpu_2_2_gclk_start = ~cpu_2_2_clk_gate_en;

  
    four_stage_processor cpu_2_2 (
        //.clk(clk),
        .clk(cpu_2_2_gclk),

        .reset(reset),
        .inst_in(node_2_2_inst_in),
        .d_in(node_2_2_d_in),
        .pc_out(node_2_2_pc_out),
        .d_out(node_2_2_d_out),
        .addr_out(node_2_2_addr_out),
        .memWrEn(node_2_2_memWrEn),
        .memEn(node_2_2_memEn),
        .nicEn(nicEn_2_2),
        .nicWrEn(nicEnWR_2_2),
        .addr_nic(nic_addr_2_2),
        .d_in_nic(nic_d_in_2_2),
        .d_out_nic(nic_d_out_2_2)
    );
      router router_3_2 (
    .clk (clk),
    .reset(reset),
    .router_position(0), 
    .polarity_out(net_polarity_32),
    // Left side
    .cwsi(cw_so_22_si_32), .cwri(cw_ro_22_ri_32), .cwdi(cw_do_22_di_32),
    .ccwso(ccw_si_22_so_32), .ccwro(ccw_ri_22_ro_32), .ccwdo(ccw_di_22_do_32),

    // Right side, no connections
    .ccwsi(), .ccwri(), .ccwdi(), .cwso(), .cwro(), .cwdo(),

    // Bottom, no connections
    .snsi(snsi_3_2), .snri(snri_3_2), .sndi(sndi_3_2), .nsso(nsso_3_2), .nsro(nsro_3_2), .nsdo(nsdo_3_2),

    // Top side
    .snso(snso_3_2), .snro(snro_3_2), .sndo(sndo_3_2), .nssi(nssi_3_2), .nsri(nsri_3_2), .nsdi(nsdi_3_2),

    // PE input/output to NIC
    .pesi(net_so_pesi_32), .pedi(net_do_pedi_32), .peri(net_ro_peri_32),
    .peso(net_si_peso_32), .pedo(net_di_pedo_32), .pero(net_ri_pero_32)
  );

  // NIC module instantiation for 0_2
  nic #(
      .PACKET_WIDTH(PACKET_WIDTH)
  ) nic_3_2 (
      .clk(clk),
      .reset(reset),
  
      // CPU-NIC Interface
      .addr(nic_addr_3_2),
      .d_in(nic_d_in_3_2),
      .d_out(nic_d_out_3_2),
      .nicEn(nicEn_3_2),
      .nicEnWR(nicEnWR_3_2),

  
      // Router-NIC Interface
      .net_si(net_si_peso_32),
      .net_so(net_so_pesi_32),
      .net_ri(net_ri_pero_32),
      .net_ro(net_ro_peri_32),
  
      .net_di(net_di_pedo_32),
      .net_do(net_do_pedi_32),
      .net_polarity(net_polarity_32)
  );
    reg [3:0] cpu_3_2_gclk_count;
    wire cpu_3_2_gclk_start;
    reg cpu_3_2_gclk_count_done;
    
    always @(posedge clk or posedge reset) begin
        if (reset) begin
            cpu_3_2_gclk_count <= 3'd0;
            cpu_3_2_gclk_count_done <= 1'b0;
        end else if (cpu_3_2_gclk_start) begin
            if (cpu_3_2_gclk_count < 3'd5) begin
                cpu_3_2_gclk_count <= cpu_3_2_gclk_count + 1;
                cpu_3_2_gclk_count_done <= 1'b0;
            end else begin
                cpu_3_2_gclk_count_done <= 1'b1;
            end
        end else begin
            cpu_3_2_gclk_count <= 3'd0;
            cpu_3_2_gclk_count_done <= 1'b0;
        end
    end

    wire cpu_3_2_clk_gate_en;
    wire cpu_3_2_gclk;
    assign cpu_3_2_clk_gate_en = !(node_3_2_inst_in == 64'b0);

    clk_gate_latch cpu_3_2_clkgate (
        .CLK(clk), 
        .EN(cpu_3_2_clk_gate_en), 
        .GCLK(cpu_3_2_gclk)
    );

    assign cpu_3_2_gclk_start = ~cpu_3_2_clk_gate_en;

four_stage_processor cpu_3_2 (
    //.clk(clk),
    .clk(cpu_3_2_gclk),

    .reset(reset),
    .inst_in(node_3_2_inst_in),
    .d_in(node_3_2_d_in),
    .pc_out(node_3_2_pc_out),
    .d_out(node_3_2_d_out),
    .addr_out(node_3_2_addr_out),
    .memWrEn(node_3_2_memWrEn),
    .memEn(node_3_2_memEn),
    .nicEn(nicEn_3_2),
    .nicWrEn(nicEnWR_3_2),
    .addr_nic(nic_addr_3_2),
    .d_in_nic(nic_d_in_3_2),
    .d_out_nic(nic_d_out_3_2)
);

endmodule