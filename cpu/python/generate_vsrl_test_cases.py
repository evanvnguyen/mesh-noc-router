...
def shift_chunks_right(a_chunks, shift_vals, chunk_bits, disregard_bounds=False):
    mask = (1 << chunk_bits) - 1
...
def format_binary_64(chunks, chunk_bits):
    ...
    # Dynamic grouping based on chunk size
    if chunk_bits == 8:
        group = 8
    elif chunk_bits == 16:
        group = 16
...
def shift_registers_verbose(WW, summary_rows, log_func, simple=False, show_binary=False, generate_verilog=False, verilog_calls=None, disregard_bounds=False):
    ...
    result_chunks = shift_chunks_right(a_chunks, shift_vals, chunk_bits, disregard_bounds=disregard_bounds)
    ...
    if show_binary:
        log_func("\nBinary breakdown (64 bits, _ every chunk size):")
...
# === GUI Setup ===
root = tk.Tk()
root.title("Variable Shift Right Test Generator")
