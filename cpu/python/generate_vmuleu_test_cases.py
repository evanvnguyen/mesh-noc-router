...
def run_vmuleu_for_ww(WW, text_widget, simple=False, verbose=False, verilog_calls=None):
    ...
    if WW == '00':
        ...
        result_chunks = [(a_chunks[i] * b_chunks[i]) & 0xFFFFFFFF for i in range(0, len(a_chunks), 2)]
    ...
    if verilog_calls is not None:
        ...
        verilog_calls.append((ww_comment, f"execute_vmuleu(\n"
                                          f"    64'h{reg_a_hex},\n"
                                          f"    64'h{reg_b_hex},\n"
                                          f"    64'h{result_hex},\n"
                                          f"    2'b{WW}\n);"))
...
def run_all_ww(text_widget, simple=False, verbose=False, generate_verilog=False):
    ...
    for ww in ['00', '01', '10']:
        run_vmuleu_for_ww(ww, text_widget, simple=simple, verbose=verbose, verilog_calls=verilog_calls if generate_verilog else None)
