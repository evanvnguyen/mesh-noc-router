# Full corrected script with proper 64-bit result packing for WW=00
...
def annotate_decimal_chunks(chunks):
    return '_'.join(f"({x})" if i % 2 == 0 else str(x) for i, x in enumerate(chunks))
...
def build_64bit_result_WW00(a_chunks, b_chunks):
    result_chunks = []
    used_pairs = 0
    ...
    result_val = 0
    for val in result_chunks:
        result_val = (result_val << 16) | val
    return result_val, result_chunks

def run_vsqeu_for_ww(WW, text_widget, simple=False, verbose=False, verilog_calls=None):
    ...
    if WW == '00':
        result_val, result_chunks = build_64bit_result_WW00(a_chunks, b_chunks)
    elif WW == '01':
        result_chunks = [(a_chunks[i] ** 2) & 0xFFFFFFFF for i in range(0, len(a_chunks), 2)]
        result_val = (result_chunks[0] << 32) | result_chunks[1]
    elif WW == '10':
        result_val = (a_chunks[0] ** 2) & 0xFFFFFFFFFFFFFFFF
    ...
    if verilog_calls is not None:
        ww_comment = {
            '00': "// WW = 00 → 8b chunks (even indices only), result = 16b chunks",
            '01': "// WW = 01 → 16b chunks (even indices only), result = 32b chunks",
            '10': "// WW = 10 → 32b chunks (even index only), result = 64b chunk"
        }[WW]

        reg_a_hex = format_flat_hex(a_chunks, chunk_bits)
        reg_b_hex = format_flat_hex(b_chunks, chunk_bits)
        result_hex = f"{result_val:016x}"
        verilog_calls.append((ww_comment, f"execute_vsqeu(\n"
                                          f"    64'h{reg_a_hex},\n"
                                          f"    64'h{result_hex},\n"
                                          f"    2'b{WW}\n);"))
...
def run_all_ww(text_widget, simple=False, verbose=False, generate_verilog=False):
    ...
    for ww in ['00', '01', '10']:
        run_vsqeu_for_ww(ww, text_widget, simple=simple, verbose=verbose, verilog_calls=verilog_calls if generate_verilog else None)
